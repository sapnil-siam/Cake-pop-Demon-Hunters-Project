...
CORNER_DEMON_SIZE = 60   # size in pixels (square)
CORNER_SPAWN_INTERVAL = 3.0  # seconds between corner demon spawns (per corner)
CORNER_DEMON_LIFETIME = 6.0  # max lifetime if untouched

FALLING_DEMON_W = 80
FALLING_DEMON_H = 80
FALLING_SPAWN_INTERVAL = 7.0  # seconds between falling demon spawns
FALLING_SPEED = 180  # pixels per second

MOTION_THRESHOLD = 25  # binary threshold for motion detection
MIN_MOTION_AREA = 250  # minimum contour area to count as motion
...
# Load Haar cascade for face detection (bundled with opencv-python)
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
if face_cascade.empty():
    raise RuntimeError("Could not load Haar cascade. Make sure opencv-python is installed correctly.")
